    ...
    def render(self, data, accepted_media_type=None, renderer_context=None):
        if renderer_context:
            ...
            if response.headers["Content-Type"] == self.media_type:
                ...
                res_data = {}
                # 只返回了 ResponseCodes 枚举类，没有数据
                if isinstance(data, Enum):
                    code, msg = data.value
                # 返回了 ResponseCodes 枚举类和数据
                elif isinstance(data, tuple):
                    if isinstance(data[0], Enum):
                        code, msg = data[0].value
                        res_data = data[1]
                    else:
                        code, msg = ResponseCodes.OK.value
                        res_data = data
                # 只返回了数据
                else:
                    code, msg = ResponseCodes.OK.value
                ...
                if response.status_code == status.HTTP_401_UNAUTHORIZED:
                    ...
                    code, msg = ResponseCodes.LOGIN_EXPIRED.value
                elif response.status_code == status.HTTP_400_BAD_REQUEST:
                    response.status_code = status.HTTP_200_OK
                    code, msg = ResponseCodes.PARAM_ERROR.value
                elif response.status_code == status.HTTP_403_FORBIDDEN:
                    response.status_code = status.HTTP_200_OK
                    code, msg = ResponseCodes.PERMISSION_DENIED.value
